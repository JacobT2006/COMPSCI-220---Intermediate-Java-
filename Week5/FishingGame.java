...
public class FishingGame {
	...
	public static void main(String[] args) {
		...
		final Die die = new Die(6); // create 6 sided dice
		...
		while (userInput.equalsIgnoreCase("Y")) {
			...
			System.out.println("face number: " + die.getValue());
			
			//call getCaught() method to get the random item that user caught
			String item_caught = getCaught(die.getValue(), count);
		...
		if (numGames > 0) {
			...
			getResult(totalScore, count, numGames);
	...
	public static void getResult(int totalScore, int[] count, int numGames) {
		...
		System.out.println("\nYou played the game "  + numGames + " time(s).");
		System.out.println("Your total score: "  + totalScore + ".");
		for (int i = 0; i < 6; i++) {
			System.out.println("You caught "  +count[i] +  " "  + ITEMS[i] + ".");
		}
